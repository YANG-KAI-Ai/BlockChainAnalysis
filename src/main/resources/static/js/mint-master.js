...
$(document).ready(function () {
    ...
    var tableHeader = ["Name", "dappId", "Volume", "Volume PercentageChange", "avg Price","Avg Price Percentage Change",  "traders", "Traders Percentage Change"]
    var index = 0;
...
//        $("#one-one-section .col-section-main-table-header").append(template);
    ...
    function oneOneSectionLoadData() {
            ...
            queryData(baseUrl+"showNFTDataList", {}, function (resp) {
                resp = $.parseJSON(resp);
                if (!resp || !resp.length){
                    console.log("获取列表为空")
                    return
                ...
                for (const r of resp) {
                    ...
                    template = template.replace(tableHeader[0], r.name);
                    template = template.replace(tableHeader[1], r.dappId);
                    template = template.replace(tableHeader[2], r.volume.toFixed(2));
                    template = template.replace(tableHeader[3], r.volumePercentageChange.toFixed(2));
                    template = template.replace(tableHeader[4], r.avgPrice.toFixed(6));
                    template = template.replace(tableHeader[5], r.avgPricePercentageChange.toFixed(2));
                    template = template.replace(tableHeader[6], r.traders);
                    template = template.replace(tableHeader[7], r.tradersPercentageChange.toFixed(2));
